...
**z-code** 是一个用于生成 Markdown 格式的项目文档的工具，专门为 AI 项目输入提供简化的项目结构和代码内容展示。它通过用户自定义`config.ini`需要读取项目目录中的文件和文件夹结构，并根据用户自定义的黑名单文件 `blacklist.ini` 进行过滤，生成包含项目结构和代码内容的 `.md` 文件。
